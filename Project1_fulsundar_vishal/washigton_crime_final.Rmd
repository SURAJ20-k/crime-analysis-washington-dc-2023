...
# Open a larger PNG device to save the plot
png("correlation_heatmap.png", width=2000, height=2000, res=150)

# Create the correlation plot with adjustments
corrplot(cor_matrix, 
         method="circle",               # Use circles to represent correlations
         type="upper",                  # Display only the upper triangle of the matrix
         order="hclust",                # Use hierarchical clustering for ordering
         tl.col="black",                # Label color
         tl.srt=45,                     # Rotate text labels
         addCoef.col="black",           # Add correlation coefficients to the plot
         number.cex=0.7,                # Adjust the size of numbers
         col=brewer.pal(n=8, name="RdBu"), # Color palette for better contrast
         diag=FALSE,                    # Exclude the diagonal from the plot
         cl.lim=c(-1, 1),               # Set the color scale limits
         cex.main=1.5,                  # Increase the title size
         cex.lab=1.2,                   # Adjust label sizes
         cex.axis=1.2)                  # Adjust axis label size

# Close the PNG device after rendering the plot
dev.off()
...
selected_features <- data %>%
  select(`ANC`, `PSA`, `X`, `Y`, `LATITUDE`, `LONGITUDE`, 
         `OFFENSEASSAULT W/DANGEROUS WEAPON`, 
         `OFFENSEMOTOR VEHICLE THEFT`, 
         `OFFENSEROBBERY`, 
         `WARD2`, `WARD3`, `WARD5`, `WARD6`, 
         `METHODGUN`)
