...
data$Month <- month(data$REPORT_DAT)  # Extract month
data$Day <- day(data$REPORT_DAT)      # Extract day
...
# Select a subset of variables to visualize
...
# Increase plot resolution for clarity
png("correlation_plot.png", width=1500, height=1200, res=150)
corrplot(cor_matrix, method="circle", type="upper", order="hclust", 
         tl.col="black", tl.srt=45, diag=FALSE, addCoef.col="black", number.cex=0.7)
dev.off()

        # Hide the diagonal (perfect correlations with themselves)
...
# 1. Prepare the Data
# Let's assume your data is named 'data_selected' and the target variable is OFFENSEMOTOR VEHICLE THEFT
# Also, I'm assuming 'data_selected' is already cleaned (e.g., missing values removed)

# Check the column names in the data_selected dataset
colnames(data_selected)

# 2. Rename the 'OFFENSEMOTOR VEHICLE THEFT' column for easier referencing
colnames(data_selected)[colnames(data_selected) == "OFFENSEMOTOR VEHICLE THEFT"] <- "OFFENSEMOTOR_VEHICLE_THEFT"

# Convert the 'OFFENSEMOTOR_VEHICLE_THEFT' column to a factor (binary classification)
data_selected$OFFENSEMOTOR_VEHICLE_THEFT <- as.factor(data_selected$OFFENSEMOTOR_VEHICLE_THEFT)

# 3. Feature Engineering (Scaling Continuous Variables)
# Scale continuous variables (X, Y, LATITUDE, LONGITUDE) for models sensitive to scale
scaled_data <- data_selected
scaled_data[, c("X", "Y", "LATITUDE", "LONGITUDE")] <- scale(data_selected[, c("X", "Y", "LATITUDE", "LONGITUDE")])
...
# 5. Build the Model (Logistic Regression)
# Using logistic regression to predict 'OFFENSEMOTOR_VEHICLE_THEFT'
model <- glm(OFFENSEMOTOR_VEHICLE_THEFT ~ ANC + PSA + X + Y + LATITUDE + LONGITUDE + WARD2 + WARD3 + WARD5 + WARD6, 
             data = train_data, 
             family = binomial)
...
# Convert probabilities to binary outcome (0 or 1) using a threshold of 0.5
predictions <- ifelse(pred_probs > 0.5, 1, 0)
predictions <- as.factor(predictions)

# 7. Evaluate the Model
# Confusion Matrix to assess accuracy, precision, recall, and F1-score
library(caret)
confusion_matrix <- confusionMatrix(predictions, test_data$OFFENSEMOTOR_VEHICLE_THEFT)
print(confusion_matrix)

# Additional evaluation metrics like ROC-AUC
library(pROC)
roc_curve <- roc(test_data$OFFENSEMOTOR_VEHICLE_THEFT, pred_probs)
plot(roc_curve)
auc(roc_curve)
...
# Extracting accuracy from confusion matrix
...
# Plot ROC curves
plot(roc_train, col = "blue", main = "ROC Curve: Test")
lines(roc_test, col = "red")
legend("bottomright", legend = c("Train", "Test"), col = c("blue", "red"), lty = 1)
...
# Define a grid for hyperparameter tuning, only including 'mtry'
tune_grid <- expand.grid(
  mtry = c(1, 2, 3)               # Number of variables randomly selected at each split
)

# Train the Random Forest model with updated hyperparameters
set.seed(42)
rf_model <- train(
  formula,
  data = train_data,
...
train_preds <- predict(final_rf_model, train_data, type = "response")  # Use "response" for classification
test_preds <- predict(final_rf_model, test_data, type = "response")    # Use "response" for classification

# Check the predictions
head(train_preds)
head(test_preds)

# Ensure 'METHODGUN' is a factor in both train and test data
train_data$METHODGUN <- as.factor(train_data$METHODGUN)
test_data$METHODGUN <- as.factor(test_data$METHODGUN)


# Check column names in both train_data and test_data
cat("Train Data Columns:\n")
...
# Evaluate model performance
train_preds <- predict(final_rf_model, train_data, type = "response")  # Use "raw" for classification
test_preds <- predict(final_rf_model, test_data, type = "response")    # Use "raw" for classification

# Confusion matrix (train and test data)
conf_matrix_train <- confusionMatrix(train_preds, train_data$METHODGUN)
conf_matrix_test <- confusionMatrix(test_preds, test_data$METHODGUN)
...
# ROC and AUC
train_probs <- predict(final_rf_model, train_data, type = "prob")[, 2]
test_probs <- predict(final_rf_model, test_data, type = "prob")[, 2]

# Train ROC
train_roc <- roc(train_data$METHODGUN, train_probs)
auc_train <- auc(train_roc)

# Test ROC
test_roc <- roc(test_data$METHODGUN, test_probs)
...
```{r}
# Sample data: accuracy values for Model 1 and Model 3 for train and test sets
accuracy_model1_train <- c(0.983)  # Example values for Model 1 (train)
accuracy_model1_test <- c(0.939)   # Example values for Model 1 (test)
accuracy_model3_train <- c(0.998)  # Example values for Model 3 (train)
accuracy_model3_test <- c(0.942)   # Example values for Model 3 (test)

# Combine the data into two groups
accuracy_model1 <- c(accuracy_model1_train, accuracy_model1_test)
accuracy_model3 <- c(accuracy_model3_train, accuracy_model3_test)

# Perform One-Way ANOVA
anova_result <- aov(c(accuracy_model1, accuracy_model3) ~ factor(c(rep(1, length(accuracy_model1)), rep(2, length(accuracy_model3)))))
summary(anova_result)



# Paired t-test between Model 1 and Model 3 on accuracy (for train and test)
t_test_result <- t.test(accuracy_model1, accuracy_model3, paired = TRUE)
print(t_test_result)




# Wilcoxon signed-rank test for Model 1 and Model 3 on accuracy (train and test)
wilcoxon_result <- wilcox.test(accuracy_model1, accuracy_model3, paired = TRUE)
print(wilcoxon_result)




# Example data for multiple metrics (accuracy, sensitivity, specificity)
accuracy_model1 <- c(0.983, 0.939)
sensitivity_model1 <- c(0.92, 0.85)
specificity_model1 <- c(0.95, 0.91)

accuracy_model3 <- c(0.998, 0.942)
sensitivity_model3 <- c(0.93, 0.87)
specificity_model3 <- c(0.96, 0.93)

# Combine the data into a data frame
df <- data.frame(
  accuracy = c(accuracy_model1, accuracy_model3),
  sensitivity = c(sensitivity_model1, sensitivity_model3),
  specificity = c(specificity_model1, specificity_model3),
  model = factor(rep(c("Model 1", "Model 3"), each = 2))
)
...
```
# result 
no significant difference between the models, you can conclude that Model 1 and Model 3 are statistically similar in terms of accuracy based on the current data and metrics used.
...
# Convert the REPORT_DAT column to Date type if it's not already
data_selected$REPORT_DAT <- as.Date(data_selected$REPORT_DAT, format="%Y-%m-%d")

# Extract year and month from REPORT_DAT to create a "Month" variable
data_selected$YearMonth <- format(data_selected$REPORT_DAT, "%Y-%m")

# Aggregate the data by month, calculating the mean of ANC (or another variable) per month
monthly_data <- data_selected %>%
  group_by(YearMonth) %>%
  summarise(ANC = mean(ANC))

# Convert to time series (monthly data)
ts_data_monthly <- ts(monthly_data$ANC, frequency=48, start=c(2023, 1))  # 12 for monthly data

# Plot the time series to visualize it
plot(ts_data_monthly, main="ANC Monthly Time Series (2023)", ylab="ANC", xlab="Month")
...
# Fit the ETS model
ets_model_monthly <- ets(ts_data_monthly)

# Summary of the ETS model
summary(ets_model_monthly)

# Forecast for the next 24 months (2024 and 2025)
forecast_ets_monthly <- forecast(ets_model_monthly, h=24)  # 24 months for 2024 and 2025
...
```{r}
# Plot the ETS forecast for 24 months (2024 and 2025)
plot(forecast_ets_monthly, main="ETS Monthly Forecast for 2024 and 2025")

# Print the forecasted values
forecast_ets_monthly
...
# Assuming 'ts_data_monthly' is your time series object with monthly data

# Fit a SARIMA model
sarima_model <- auto.arima(ts_data_monthly, seasonal = TRUE, stepwise = TRUE, approximation = FALSE)
