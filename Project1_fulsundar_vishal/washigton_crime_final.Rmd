...
trainIndex <- createDataPartition(data_selected$METHODGUN, p = .8, 
                                  list = FALSE, 
                                  times = 1)

train_data <- data_selected[trainIndex,]
test_data  <- data_selected[-trainIndex,]
...
rf_model <- randomForest(METHODGUN ~ Month + Day + ANC + PSA + X + Y + 
                         OFFENSEASSAULT_W_DANGEROUS_WEAPON + OFFENSEMOTOR_VEHICLE_THEFT + 
                         OFFENSEROBBERY + WARD2 + WARD3 + WARD5 + WARD6 + WARD7 + WARD8 + 
                         SeasonSpring + SeasonSummer + SeasonWinter, data = train_data)
...
```{r}
# Get predicted probabilities (for ROC and AUC) on the training data
pred_probs_train <- predict(rf_model, train_data, type = "prob")

# Extract probabilities for the positive class (METHODGUN = 1)
prob_pos_class_train <- pred_probs_train[, 2]

# Calculate ROC curve for the training data
roc_curve_train <- roc(train_data$METHODGUN, prob_pos_class_train)

# Plot the ROC curve for the training data
plot(roc_curve_train, main = "ROC Curve (Train Data)", col = "blue", lwd = 2)

# Calculate AUC for the training data
auc_value_train <- auc(roc_curve_train)

# Print AUC value
print(paste("AUC (Train Data): ", auc_value_train))
```
...
confusionMatrix(predictions, test_data$METHODGUN)


# Calculate accuracy
accuracy <- mean(predictions == test_data$METHODGUN)
print(paste("Accuracy: ", accuracy))


# Tuning the Random Forest model (optional)
rf_tune <- train(METHODGUN ~ Month + Day + ANC + PSA + X + Y + 
                 OFFENSEASSAULT_W_DANGEROUS_WEAPON + OFFENSEMOTOR_VEHICLE_THEFT + 
                 OFFENSEROBBERY + WARD2 + WARD3 + WARD5 + WARD6 + WARD7 + WARD8 + 
...
# The probabilities for the positive class (METHODGUN = 1) are in the second column
prob_pos_class <- pred_probs[, 2]

# Calculate ROC curve
roc_curve <- roc(test_data$METHODGUN, prob_pos_class)
...
# Define a grid for hyperparameter tuning (only mtry allowed for randomForest)
tune_grid <- expand.grid(
  mtry = c(2, 4, 6) # Number of variables randomly selected at each split
)
...
train_preds <- predict(final_rf_model, train_data)
