...
abline(a = 0, b = 1, lty = 2, col = "gray")  # Add diagonal line for random classifier
...
ggplot(data_selected, aes(x = METHODGUN)) +
  geom_bar(fill = "steelblue") +
  ggtitle("Class Distribution Before Oversampling") +
  xlab("METHODGUN") +
  ylab("Count") +
  theme_minimal()

# Class distribution after oversampling
ggplot(balanced_data, aes(x = METHODGUN)) +
  geom_bar(fill = "darkorange") +
  ggtitle("Class Distribution After Oversampling") +
  xlab("METHODGUN") +
  ylab("Count") +
  theme_minimal()
...
# Summarize misclassified instances
...
# Summarize correctly classified instances
...
# Plot ROC curves
plot(roc_train, col = "blue", main = "ROC Curve: Test VS TRAIN ")
lines(roc_test, col = "red")
legend("bottomright", legend = c("Train", "Test"), col = c("blue", "red"), lty = 1)
...
# ANNOVA test for model selection 
```{r}
...
# Aggregate crime data by Year, Month, and geographic area (e.g., by districts or bins)
...
# Convert aggregated data to time series for a specific region
crime_ts <- ts(crime_geo_data$Crimes, start = c(2023, 1), frequency = 12)

# Fit a SARIMA model or ETS model
...
# Plot the forecast
plot(sarima_forecast, main = "Crime Forecast for 2024-2025 (Geospatial)")
...
# Print the forecasted values
print(forecast_ets_monthly)

# Add a summary for the forecast plot
...
# Add a summary for the SARIMA plot
...
# Create a data frame for the forecasted values and dates
...
#EDA-GUN 
```{r}
gun_data <- crime_data2 %>% filter(METHOD == "GUN")
...
gun_data %>%
  group_by(SHIFT) %>%
  summarise(Count = n()) %>%
  ggplot(aes(x = SHIFT, y = Count, fill = SHIFT)) +
  geom_bar(stat = "identity") +
  labs(title = "Gun-Related Crimes by Shift", x = "Shift", y = "Count")
...
gun_data %>%
  ...
  summarise(Count = n()) %>%
  arrange(desc(Count)) %>%
  ggplot(aes(x = reorder(DISTRICT, Count), y = Count, fill = DISTRICT)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(title = "Gun-Related Crimes by District", x = "District", y = "Count")
...
gun_data %>%
  ...
  summarise(Count = n()) %>%
  arrange(desc(Count)) %>%
  ggplot(aes(x = reorder(NEIGHBORHOOD_CLUSTER, Count), y = Count, fill = NEIGHBORHOOD_CLUSTER)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(title = "Gun-Related Crimes by Neighborhood Cluster", x = "Neighborhood Cluster", y = "Count")
...
gun_data %>%
  ...
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(title = "Offense Types in Gun-Related Crimes", x = "Offense", y = "Count")
...
gun_data %>%
  group_by(WARD) %>%
  summarise(Count = n()) %>%
  ggplot(aes(x = reorder(WARD, Count), y = Count, fill = WARD)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(title = "Gun-Related Crimes by Ward", x = "Ward", y = "Count")
...
gun_data %>%
  ...
  geom_bar(stat = "identity") +
  labs(title = "Gun-Related Crimes by Season", x = "Season", y = "Count")
...
ggplot(gun_data, aes(x = LONGITUDE, y = LATITUDE)) +
  geom_density2d() +
  stat_density2d(aes(fill = ..level..), geom = "polygon") +
  scale_fill_viridis_c() +
  labs(title = "Heatmap of Gun-Related Crimes", x = "Longitude", y = "Latitude")
...
gun_data %>%
  ...
  summarise(Count = n()) %>%  # Summarize the count of crimes
  arrange(match(Month, month.abb)) %>%  # Arrange months in proper order
  ggplot(aes(x = Month, y = Count, fill = Month)) +
  geom_bar(stat = "identity") +  # Create a bar plot
  labs(
    title = "Gun-Related Crimes in 2023 by Month",
    x = "Month",
    y = "Count"
  ) +
  theme_minimal() +  # Use a minimal theme
  scale_fill_brewer(palette = "Set3")  # Add a color palette
