...
levels(predictions)
levels(testData$METHOD)

train_data_scaled$METHOD <- factor(train_data_scaled$METHOD, levels = c("GUN", "KNIFE", "OTHERS"))
rf_model1 <- randomForest(formula = METHOD ~ Day + LATITUDE + LONGITUDE + Month + WARD + ANC + OFFENSE, 
...
confusion_matrix <- table(predictions, testData$METHOD)
print(confusion_matrix)

# Calculate accuracy of the model
accuracy <- sum(predictions == testData$METHOD) / nrow(testData)
...
```
```{r}
# Check the structure of the dataset to identify a suitable dependent variable
str(trainData2)

library(AER)  # For dispersion test

# Ensure that METHOD is a count variable (Poisson regression requires a count outcome)
# If METHOD is not a count, you need to use another count variable or rethink the model.
# Load required libraries
library(AER)  # For dispersion test

# Check the structure of the data
str(trainData2)
summary(trainData2$METHOD)  # Replace METHOD with the actual dependent variable name

# Convert METHOD to numeric if it is a factor
if (is.factor(trainData2$METHOD)) {
  trainData2$METHOD <- as.numeric(as.character(trainData2$METHOD))
}
if (is.factor(testData2$METHOD)) {
  testData2$METHOD <- as.numeric(as.character(testData2$METHOD))
}

# Handle missing or invalid values
trainData2 <- trainData2[!is.na(trainData2$METHOD) & trainData2$METHOD >= 0, ]
testData2 <- testData2[!is.na(testData2$METHOD) & testData2$METHOD >= 0, ]

levels(trainData$SHIFT)
levels(trainData$WARD)
levels(trainData$DISTRICT)
levels(trainData$OFFENSE)
levels(trainData$Season)
levels(trainData$METHOD)
table(trainData$METHOD)
...
multinom_model <- multinom(
  METHOD ~ SHIFT + WARD + DISTRICT + OFFENSE + Season,
  data = trainData
)
...
# Perform a dispersion test to check for overdispersion
dispersion_test <- dispersiontest(multinom_model)

# Print the results of the dispersion test
print(dispersion_test)

# If overdispersion is detected, fit a quasi-Poisson model
if (dispersion_test$p.value < 0.05) {
  ...
  # Check the summary of the quasi-Poisson model
  summary(quasi_poisson_model)
}

# Predict on the test dataset
predictions <- predict(multinom_model, newdata = testData)

# Confusion matrix
conf_matrix <- table(predictions, testData$METHOD)
...
crime_data2$SHIFT <- as.numeric(crime_data2$SHIFT)
crime_data2$METHOD <- as.numeric(crime_data2$METHOD)
crime_data2$OFFENSE <- as.numeric(crime_data2$OFFENSE)
crime_data2$WARD <- as.numeric(crime_data2$WARD)
crime_data2$ANC <- as.numeric(crime_data2$ANC)
crime_data2$DISTRICT <- as.numeric(crime_data2$DISTRICT)
crime_data2$Season <- as.numeric(crime_data2$Season)

# Converting the 'Year' and 'Month' variables to numeric
crime_data2$Year <- as.numeric(crime_data2$Year)
crime_data2$Month <- as.numeric(factor(crime_data2$Month, levels = c("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec")))
...
corrplot(corr_matrix, method = "number", type = "full", number.cex = 0.7)

corr_matrix <- cor(crime_data2[, sapply(crime_data2, is.numeric)], use = "complete.obs")
print(corr_matrix)
...
numeric_data <- crime_data2[, sapply(crime_data2, is.numeric)]
...
write.csv(corr_matrix, "filtered_correlation_matrix.csv")
...
crime_data2$SHIFT <- as.factor(crime_data2$SHIFT)
crime_data2$METHOD <- as.factor(crime_data2$METHOD)
crime_data2$OFFENSE <- as.factor(crime_data2$OFFENSE)
crime_data2$WARD <- as.factor(crime_data2$WARD)
crime_data2$DISTRICT <- as.factor(crime_data2$DISTRICT)
crime_data2$Season <- as.factor(crime_data2$Season)
...
set.seed(123)  # For reproducibility
trainIndex <- createDataPartition(crime_data2$OFFENSE, p = 0.8, list = FALSE)
trainData <- crime_data2[trainIndex, ]
testData <- crime_data2[-trainIndex, ]
...
```{r}
# Normalize continuous variables (LATITUDE, LONGITUDE, ANC)
preProc <- preProcess(trainData[, c("LATITUDE", "LONGITUDE", "ANC")], method = c("center", "scale"))
trainData_scaled <- predict(preProc, trainData[, c("LATITUDE", "LONGITUDE", "ANC")])
testData_scaled <- predict(preProc, testData[, c("LATITUDE", "LONGITUDE", "ANC")])
...
# Train the KNN model (using k=5 as an example)
knn_pred <- knn(train = trainData_scaled, test = testData_scaled, 
                cl = trainData$OFFENSE, k = 9)
...
confusionMatrix(knn_pred, testData$OFFENSE)
...
scaler <- preProcess(train_data[, c("LATITUDE", "LONGITUDE", "Day", "Month", "WARD","ANC","METHOD")], method = "scale")
train_data_scaled <- predict(scaler, train_data)
test_data_scaled <- predict(scaler, test_data)

# Train a Random Forest Model
rf_model <- randomForest(OFFENSE ~ Day + LATITUDE + LONGITUDE + Month + WARD+ANC+ METHOD, data = train_data_scaled)
...
confusionMatrix(predictions, test_data_scaled$OFFENSE)
