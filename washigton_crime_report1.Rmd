...
# Distribution of different offenses by shift----
...
```
# Distribution of  offenses method  by shift---
...
```
# Distrubution of offences bt ward-
...
#1) Chi-Square Test Results - (shift and offence type)
...
# Fisher-test (shift and offence type)
...
```
# Chi-squard test( shift vs method)
...
```{r}
crime_data$WARD <- as.character(crime_data$WARD)

crime_data$WARD <- paste("Ward", crime_data$WARD)
...
# Convert the WARD column to character and then to numeric if needed
crime_data2$WARD <- as.numeric(as.character(crime_data2$WARD))

merged_data <- crime_data2 %>%
  left_join(cluster,by = c("NEIGHBORHOOD_CLUSTER" = "NAME"))



merged_data <- merged_data %>%
  left_join(police_stations,by = c("WARD" = "OBJECTID"))
head(merged_data)
...
# Example of creating a simple map of crime incidents
ggplot() +
  geom_point(data = crime_data2, aes(x = LONGITUDE, y = LATITUDE, color = OFFENSE), size = 1.5, alpha = 0.7) +
  geom_point(data = police_stations, aes(x = LONGITUDE, y = LATITUDE), color = "red", size = 3, shape = 3) +  # police stations
  labs(title = "Crime Incidents in Washington, DC",
       x = "Longitude",
       y = "Latitude",
